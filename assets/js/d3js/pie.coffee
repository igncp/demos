ready = ()->
  ...
  d3.json("/data/d3js/pie/data.json", (error, data)->   
    ...
    labels = slices.filter((d)-> d.endAngle - d.startAngle > .2).append("text").attr({dy:".35em", "text-anchor": 'middle'})
    .attr("transform", textTransform).style({fill: "White", font: "bold 12px Arial"}).text((d)-> d.data.val)
    ...
    d3.select('#change-data').on('click', ()->
      ...
      labels.data(pie(data)).transition().duration(1000).attr('transform', textTransform).each('start', (d)->
