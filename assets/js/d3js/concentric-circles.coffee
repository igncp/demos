ready = ()->
  d3.csv("/data/d3js/concentric-circles/data.csv", (data)->
    ...
    .style("filter", (d)-> if d.count > dataMax/3 then return "url(#drop-shadow)" else return '')
    .on("mouseover", ()->
    ...
    svg.append('text').text('Circles radius are proportional to the count of times the name appears')
    .attr('transform', 'translate(' + width/2+ ',' + (height - 10) + ')').attr('width', '20px')
  )
