ready = ()->
  ...
  d3.tsv("/data/d3js/trend-line/data.tsv", (error, data)->
    ...
    renderGraph = (()->
      ...
      svg = d3.select("#chart").text('').append("svg").attr({width: width + margin.left + margin.right, \
      ...
      svg.append("path").datum(data).transition().duration(animationTime).attrTween('d', ()-> getInterpolation(data, line)).attr("class", "line")
      ...
      svg.append("path").datum(data).transition().delay(animationTime).duration(animationTime).attrTween('d', ()-> getInterpolation(data, regression_line)).attr("class", "rline")
      svg.append("text").attr('transform', 'translate(' + width*.7+ ',' + height*.7+')').style('opacity', 0).transition(1000).delay(animationTime * 2).text('Slope: ' + lr.slope.toExponential(3)).style('opacity', 1)
    )
