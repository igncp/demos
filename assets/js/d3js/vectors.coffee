ready = ->
  ...
  svg.append('svg:defs').append('svg:marker').attr('id', 'start-arrow')
    ...
    .attr('refX', 4).attr('markerWidth', 3).attr('markerHeight', 3).attr('orient', 'auto')
    .append('svg:path').attr('d', 'M10,-5L0,0L10,5').attr('fill', '#000')
  ...
  restart = ( ->
    ...
    path.classed('selected', (d)-> d is selected_link )
    ...
    path.enter().append('svg:path').attr('class', 'link')
      .classed('selected', (d)-> return d is selected_link )
      .style('marker-start', (d)-> if d.left then return 'url(#start-arrow)' else return '')
      .style('marker-end', (d)-> if d.right then return 'url(#end-arrow)' else return '')
      .on('mousedown', (d)->
    ...
    circle.selectAll('circle').style('fill', (d)-> if d is selected_node then \
      return d3.rgb(colors(d.id)).darker().toString() else return colors(d.id))
      .classed('reflexive', (d)-> d.reflexive)

    g = circle.enter().append('svg:g')

    g.append('svg:circle').attr('class', 'node').attr('r', 12)
      .style('fill', (d)-> if d is selected_node then \
        return d3.rgb(colors(d.id)).brighter().toString() else return colors(d.id))
  ...
  mousemove = ( ->
    ...
    drag_line.attr('d', 'M' + mousedown_node.x + ',' + mousedown_node.y + 'L' + \
      d3.mouse(this)[0] + ',' + d3.mouse(this)[1]) # update drag line
  ...
  svg.on('mousedown', mousedown).on('mousemove', mousemove)
    .on('mouseup', mouseup);
