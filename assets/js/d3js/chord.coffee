ready =  ->
  ...
  charts = svg.selectAll('div').data([debits, credits]).enter().append('div')
    .style('display', 'inline-block')
    .style('width', width + 'px').style('height', height + margin.top + margin.bottom + 'px')
  ...
  setLabel = (chart, label)->
    chart.append('text')
      .text(label).attr('transform','translate(0,' + ((-1) * height / 2 - 10) + ')')
      .attr('class', 'chart-title').attr('text-anchor', 'middle')
  ...
  d3.csv('/data/d3js/chord/data.csv', (data)->
    ...
    layout = d3.layout.chord().sortGroups(d3.descending)
      .sortSubgroups(d3.descending).sortChords(d3.descending).padding(.04)
    ...
    charts.each((matrix, j)->
      ...
      svg.selectAll('path.chord').data(layout.chords).enter().append('svg:path')
        ...
        .text((d)-> d.source.value.debtor.name + ' owes ' + \
          d.source.value.creditor.name + ' $' + format(d.source.value) + 'B.')

      g = svg.selectAll('g.group').data(layout.groups).enter()
      ...
      g.append('svg:path').style('fill', (d)-> fill(d.index))
        ...
        .append('svg:title')
        .text((d)-> array[d.index].name + ' ' + (if j then 'owes' else 'is owed') \
          + ' $' + format(d.value) + 'B.')
