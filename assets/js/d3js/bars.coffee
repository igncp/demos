ready = ()->
  d3.json("/data/d3js/bars/data.json", (error, data)->   
    ...
    chart.append("g").attr({"transform": "translate(0," + floor + ")", class: 'x-axis axis'}).call(xAxis)
    .append("text").attr("transform", "translate(" +barWidth * data.length/2 + " ,0)")
    .attr('class', 'x-axis-label')
    .attr("y", 40).attr("font-size", "1.3em").style({"text-anchor": "end"})
    .text("Number")

    chart.append("g").attr({"transform": "translate(0," + floor + ")", class: 'x-axis axis'}).call(yAxis)
    ...
    redraw = (()->
      ...
      chart.select(".x-axis-label").transition().duration(500).attr("transform", "translate(" +barWidth * data.length/2 + " ,0)")
